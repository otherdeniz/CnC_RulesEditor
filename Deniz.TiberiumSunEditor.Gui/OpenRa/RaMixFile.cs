...
namespace Deniz.TiberiumSunEditor.Gui.OpenRa
{
		public sealed class RaMixFile
		{
			...
			public RaMixFile(Stream s, string filename, string[] globalFilenames)
			{
				Name = filename;
				this.s = s;
			...
			Dictionary<string, PackageEntry> ParseIndex(Dictionary<uint, PackageEntry> entries, string[] globalFilenames)
			{
				...
				if (unknown > 0)
					Debug.Write("debug", $"{Name}: failed to resolve filenames for {unknown} unknown hashes");
            ...
            public Stream GetContentFile(PackageEntry entry)
            {
