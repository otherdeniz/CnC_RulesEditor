namespace Deniz.TiberiumSunEditor.Gui.Utils.CncParser
{
    ...
    public class ShpFile
    {
        ...
        private List<ShpFrameInfo>? _shpFrameInfos;
        ...
        public int FrameCount => _shpFrameInfos?.Count ?? 0;

        public int Width => _shpFileHeader.SpriteWidth;
        public int Height => _shpFileHeader.SpriteHeight;
        ...
        public void ParseFromBuffer(byte[] buffer)
        {
            try
            {
                _shpFileHeader = new ShpFileHeader(buffer);
                _shpFrameInfos = new List<ShpFrameInfo>(_shpFileHeader.FrameCount);

                using (var memoryStream = new MemoryStream(buffer))
                {
                    memoryStream.Position = ShpFileHeader.SizeOf;

                    for (int i = 0; i < _shpFileHeader.FrameCount; i++)
                    {
                        var shpFrameInfo = new ShpFrameInfo(memoryStream);
                        _shpFrameInfos.Add(shpFrameInfo);
                    }
                }
            }
